...
严格顺序：**先检查 → 先备份 → 再清空 → 最后移动**。文件输出目录只在最后一步被移动，中间任何一步失败都不会消费用户文件。

```
1. 只读检查：文件输出目录非空，否则 NO_OUTPUT_FILES（绝不动任何东西）
2. 备份分支（先备份后动手）：
   先查游戏目录文件数 > 0，否则 GAME_DIR_EMPTY（提示具体路径，文件输出目录原封不动）
   游戏目录/* 复制(cp) → work/bak → Java 压缩 zip → 备份/时间戳.zip
     zip 确认落盘前游戏目录保持原样；失败只需清中转（复制式，无需回滚）★
   不备份分支：直接 rm -rf 游戏目录（下策）
3. 清空游戏目录（rm -rf + mkdir -p）
4. 文件输出/* mv 到游戏目录（注意是「移动」，成功后文件输出目录为空）★
5. mv 失败且有备份 → 自动从备份回滚 ★
```

> 历史坑：旧实现先把「文件输出/*」mv 进 work/out 中转再备份，备份一旦失败（如游戏目录为空）用户文件就被"吃掉"且无明确提示——已改为上面的顺序，并让 UI 按 `ReplaceResult` 枚举给出具体原因（无成品/无权限/游戏目录为空/备份失败/移动失败）。

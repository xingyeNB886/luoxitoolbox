...
private fun StatusCard(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(13.dp)) {
        // 洛茜工具箱：使用新的权限授权状态（替代原 ksuVersion 检测）
        ...
        val statusIcon = if (working) {
            Icons.Outlined.CheckCircle
        } else {
            Icons.Outlined.Warning
        }
        val statusTitle = if (working) {
            stringResource(R.string.permission_working)
        } else {
            stringResource(R.string.permission_not_working)
        }
        val statusSummary = if (working) {
            stringResource(R.string.permission_working_summary)
        } else {
            stringResource(R.string.permission_tap_to_grant)
        }
        val workingMode = if (working && grantLabel.isNotEmpty()) grantLabel else ""

        val statusTrailing: (@Composable () -> Unit)? = if (working && workingMode.isNotEmpty()) {
            {
                StatusTag(
                    label = workingMode,
